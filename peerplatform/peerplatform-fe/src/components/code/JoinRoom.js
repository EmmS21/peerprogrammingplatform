...
const JoinRoom = () => {
  let { username, setUserName, profileURL, setRoomName } =
    ...
    if (currentUser) {
      ...
      try {
        ...
        const targetRoom = roomsData.rooms.find(
          (room) => room.room_name === rooms,
        );

        if (targetRoom) {
          const twilioTokenResponse = await fetch(
            `${profileURL}voice_chat/token/${currentUser}`,
          );
  ...
  return (
    ...
    >
      <h1 style={{ color: "black" }}>
        You have been invited by {firstUser} to join a pair programming session
      </h1>
      <Input
        ...
        value={currentUser}
      />
      <Button onClick={handleFormSubmit} disabled={!currentUser}>
        Submit
      </Button>
    </div>
  );
