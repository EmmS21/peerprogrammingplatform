...
export default function ProgrammingChallenge({ query, challengeInState }) {
  const { getSolution, isLoadingSolution, setInputArr, setOutputArr } =
  ...
  useEffect(() => {
    if (challengeInState && Object.keys(challengeInState).length > 0) {
      ...
      const { explanation, exampleData } = parseExplanationAndExamples(
        challenge.extra_explain,
      );
      setChallengeData({
        ...
        examples: [
          challenge.Example2,
          challenge.Example3 || challenge.Example3,
        ].filter(Boolean),
        explanation,
        exampleData,
      });
  ...
  function parseTableData(data) {
    ...
    // Extract headers
    const headers = lines[headersStart]
      .trim()
      .split("|")
      .map((header) => header.trim());
    const textDataAfterTablePosition = lines
      .slice(0, tableEnd + 1)
      .join("\n").length;
  ...
  function parseExamplesData(data) {
    ...
    headers.forEach((header) => {
      ...
      const nextHeader = headers.find(
        (h) => data.indexOf(h + ":", startPos + header.length + 1) !== -1,
      );
      const endPos = nextHeader ? data.indexOf(nextHeader + ":") : data.length;
      if (startPos !== -1 && endPos !== -1) {
        splitData[header] = data
          .slice(startPos + header.length + 1, endPos)
          .trim();
      }
    });
    return splitData;
  ...
  function parseExplanationAndExamples(data) {
    ...
    const explanation = data
      .substring(explanationStart, exampleStart)
      .replace("Simplified Explanation:", "")
      .trim();
    const exampleData = parseExampleData(
      data.substring(exampleStart + "Some Simple Examples:".length).trim(),
    );
  ...
  const tableData = useMemo(
    () => parseTableData(challengeData.description),
    ...
    <div className="challenge" style={{ color: "white" }}>
      {isLoadingSolution ? (
        ...
        <div className="challenge" style={{ color: "white" }}>
          ...
          />
          <center>
            <Tag color="cyan">{challengeData.difficulty}</Tag>
          </center>
          <div className="problem-container">
            <RenderTableOrText data={challengeData.description} />
          </div>
          <div>
            {tableData?.textDataAfterTable
              ? formatTextForJSX(tableData.textDataAfterTable)
              : ""}
          </div>
