...
function Solutions({ challengeInState, query }) {
  let { gptresp, openModal, getSolution } = useContext(AuthContext);
  ...
  useEffect(() => {
    const fetchSolution = async () => {
      ...
      const fetchedSolution = await getSolution(
        challengeInState[0].title,
        null,
        "two",
      );
      let cleanedString = fetchedSolution
        .replace(/\\n/g, "")
        .replace(/0\\\\2/g, "0.2");
  ...
  return (
    <div style={{ zIndex: 2, position: "relative", color: "white" }}>
      {loading ? (
        ...
        <div>
          <h4>Leetcode Question: </h4>
          {solution["Leetcode Question"]}
          <h4>Algorithm</h4>
