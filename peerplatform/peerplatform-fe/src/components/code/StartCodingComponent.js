  ...
  const sendWaitingRoomUsersToRedisCache = () => {
    axios
      ...
      .then((res) => {
        ...
        axios.get(`${profileURL}users/`).then((res) => {
          const filteredUsers = res.data.filter(
            (filtered) => filtered.profile.in_waiting_room === true,
          );
          ...
          axios.post(`${profileURL}cache/`, allUserNames).then((res) => {});
          axios(config).then((res) => {
            availableOnlineUsers.current = res.data.elements;
            console.log(
              "availusers in startcoding is now",
