...
const TestComponent = () => {
    ...
    const onLoad = (editor) => {
        ...
        editor.on('click', function(e) {
            ...
            const startIndex = lines.findIndex(line => line.includes('// add code'));
            ...
            } else if (cursor.row === endIndex || (cursor.row > endIndex && cursor.row < endCommentIndex)) {
    ...
    return (
        ...
        <Menu class="w-full" pointing widths={ 5 } size={"tiny"} style={{ marginTop:0 }}>
            <Menu.Item>
                <Button onClick={() => history.push(history.location.pathname.reaplce('/tester', ''))}>Back</Button>
            </Menu.Item>
            <Menu.Item>
