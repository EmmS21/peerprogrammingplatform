...
export const AuthProvider = ({ children }) => {
  ...
  const getProfileInfo = (userId) => {
    axios.get(`${profileURL}users/${userId}`).then((res) => {
      setUser({
        ...user,
  ...
  const updateProfile = (userData, userID) => {
    axios
      .put(`${profileURL}${userID}/`, userData)
      .then((res) => {
        setUser({
          ...user,
          first_name: res.data.first_name,
  ...
  const updateProfilePic = (profilePicData, user_id) => {
    ...
    axios
      .put(`${profileURL}${user_id}/`, profilePicData, {
        headers,
      })
      .then((res) => {
  ...
  const submitCodeJudge0 = (requestBody, answers) => {
    ...
    axios
      ...
      .then((postRes) => {
        axios
          .get(`${baseURL}/${postRes.data.token}`, {
            headers,
          })
          .then((getRes) => {
            const { status, stdout, stderr } = getRes.data;
            const splitResults = stdout
              .split("\n")
              .filter((line) => line !== "");
    ...
    axios
      ...
      .then((res) => {
        ...
        if (!token) {
          ...
          setSpinnerOn(false);
          setResp(
            "Oops, this is on us, it seems we are having an issue running your code, please screenshot this and send an email to emmanuelsibandaus@gmail.com",
          );
          return;
        }
        ...
        axios
          ...
          .then((res) => {
            ...
            if (status && status.description === "Processing") {
              setResp(
                "Your code is taking too long to execute. It might contain an infinite loop or a very intensive computation.",
              );
            }
            if (!stdout) {
  ...
  async function getSolution(challenge, query = null, opt = null) {
    ...
    let leetObj = {};
    leetObj = { title: challenge["title"], description: challenge["place"] };
    if (query) {
      leetObj["query"] = query;
    }
    if (opt) {
  ...
  function setQuestion(question) {
    ...
    const { AnswerTemplate, SimplifiedExplanation, ...newChallengeData } =
      challengeData;
  ...
  function postReview() {
    ...
    axios.post(`${profileURL}update_score/`, review).then((res) => {
